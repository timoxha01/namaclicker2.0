...
from classes import (
    ...
    SongsPopouts,
    Timer,
    configure as configure_game_classes,
    draw_button_text,
    draw_wrapped_text,
)
...
from helpers import (
    choose_tama as choose_tama_helper,
    play_next_soundtrack as play_soundtrack_helper,
    update_volume as apply_volume,
)
...
VOLUME_SDTRACK = DEFAULT_SOUNDTRACK_VOLUME

runtime = create_runtime(
    width=W,
    height=H,
    caption="NamaClicker 2.0",
    icon_path="assets/images/tamas/classic.png",
    font_path=GAME_FONT,
    music_end_event=MUSIC_END_EVENT,
)
screen = runtime["screen"]
clock = runtime["clock"]
font_40 = runtime["fonts"][40]
font_30 = runtime["fonts"][30]
font_25 = runtime["fonts"][25]
font_20 = runtime["fonts"][20]
draw_loading_screen = runtime["draw_loading_screen"]
load_image = runtime["load_image"]
load_sound = runtime["load_sound"]
...
configure_game_classes(
    ...
    volume_changing_sound=volume_changing_sound,
    coins_collecting=coins_collecting,
    text_dialogue_sound=text_dialogue_sound,
    pickable_namacoin=pickable_namacoin,
    namapass_banner_alfa_acta=namapass_banner_alfa_acta,
    namapass_banner_ospuze=namapass_banner_ospuze,
    namapass_banner_trentila=namapass_banner_trentila,
    namapass_banner_vaiiya=namapass_banner_vaiiya,
)
...
def update_volume():
    ...
    apply_volume(VOLUME, sounds)

def play_next_soundtrack():
    global music_loop
    music_loop = play_soundtrack_helper(
        pygame=pygame,
        volume=VOLUME_SDTRACK,
        soundtracks=SOUNDTRACKS,
        music_loop=music_loop,
        song_popouts=song_popouts,
    )
...
choose_tama = choose_tama_helper

credits_back_button = HoverImage(long_button_img, (W // 2, H - 42.5))
achievements_back_button = HoverImage(long_button_img, (W // 2, H - 50))
settings_back_button = HoverImage(long_button_img, (W // 2, H - 50))
trentila_button = HoverImage(trentila_button_img, (303 + (186 // 2), 270))
ospuze_button = HoverImage(ospuze_button_img, (509 + (186 // 2), 270))
alfa_acta_button = HoverImage(alfa_acta_button_img, (303 + (186 // 2), 430))
vaiiya_button = HoverImage(vaiiya_button_img, (511 + (186 // 2), 430))
...
course = Course(EXCHANGE_CLICKS_PER_NAMACOIN)
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            ...
            if (
                ...
                NamaCoins = teddy_bear.buy(NamaCoins)
            ...
            if (
                ...
                NamaCoins = beluash.buy(NamaCoins)
            ...
            if (
                ...
                NamaCoins = contestant.buy(NamaCoins)
            ...
            if (seoul_bg.button_rect.collidepoint(event.pos) and mode == "backgrounds_shop"):
                if not seoul_bg.isBought:
                    NamaCoins = seoul_bg.buy(NamaCoins)
                else:
            ...
            if (kyoto_bg.button_rect.collidepoint(event.pos) and mode == "backgrounds_shop"):
                if not kyoto_bg.isBought:
                    NamaCoins = kyoto_bg.buy(NamaCoins)
                else:
                    bernal_bg.equipped = False
            ...
            if (bernal_bg.button_rect.collidepoint(event.pos) and mode == "backgrounds_shop"):
                if not bernal_bg.isBought:
                    NamaCoins = bernal_bg.buy(NamaCoins)
                else:
                    seoul_bg.equipped = False
                    kyoto_bg.equipped = False
                    bernal_bg.equip()
